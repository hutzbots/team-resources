...

{
    ...
    DcMotor tapeMeasure;
    //DcMotor tapeMeasureB; //to be used when 2 tape measures are used
    Servo buttonServo;
    Servo climberServo;
	...
	//tapeMeasureB = hardwareMap.dcMotor.get("tapeMeasureB");
	...
	//tapeMeasureB.setPower(0);
        ...
        if(gamepad2.dpad_up && climberServo.getPosition() <= 0.9){
            climberServo.setPosition(climberServo.getPosition()+0.1);
        }
        if(gamepad2.dpad_down && climberServo.getPosition() >= 0.1){
            climberServo.setPosition(climberServo.getPosition()-0.1);
        }
	if(gamepad2.y){
	    tapeMeasure.setPower(1);
	    //tapeMeasureB.setPower(1);
	}
	else if(gamepad2.a){
	    tapeMeasure.setPower(-1);
	    //tapeMeasureB.setPower(-1);
